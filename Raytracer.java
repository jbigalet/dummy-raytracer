...
import raytracer.materials.Light;
...
//    static int zoom = 4;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        world.add(new Sphere(new Vec(0f, -1000.5f, -1f), 1000f, new Lambertian(new Vec(0.6f, 0.8f, 0.5f))));
        
        world.add(new Sphere(new Vec(0f, 0f, -1f), 0.5f, new Light()));
        world.add(new Sphere(new Vec(0.5f, -0.4f, 0f), 0.1f, new Light()));
//        world.add(new Sphere(new Vec(-0.5f, -0.8f, -0.5f), 0.5f, new Light()));
...
//        for(int i=0 ; i<20 ; i++){
//            float x = 3f-6f*(float)Math.random();
//            world.add(new Sphere(
//                    new Vec(
//                        x, // x
//                        0f,      // y
//                        -8f*(float)Math.random() - (Math.abs(x) < 1.5f ? 2f : 0f)     // z
//                    ),
//
//                    0.1f, // radius
//                    
...
//                            : ( Math.random() > 0.5 ? new Light()
//                                    : new Metal(     new Vec((float)Math.random(), (float)Math.random(), (float)Math.random())))));
//        }

        for(float theta = 0 ; theta<=2*Math.PI ; theta += Math.PI/3f) {
            float x = 0.4f*(float)Math.cos(theta);
            float z = 0.4f*(float)Math.sin(theta) - 1f;
            world.add(new Sphere(
                    new Vec(
                        x, // x
                        -0.4f,      // y
            ...
            new Thread() {
                public void run() {
                    while( !pixelQueue.isEmpty() ){
                        ...
                        screen[pixel.x][pixel.y] = camera.getColorWithAntialiasing(world, pixel.x, pixel.y, 1000, 1000).toRGB(gamma);
                    }
                }
            }.start();
    }
